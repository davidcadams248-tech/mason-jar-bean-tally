// Mason Jar Bean Tally - v4 (Combined)
// Restores: Custom Beans Editor + Summary Chart
// Keeps: 3D jars, no-overlap controls, mobile tweaks, cache-busting
// Optional: Framer Motion animations if CDN loaded.
// Browser-ready (React UMD + Babel).
...
const STORAGE_KEY = "mason-jar-bean-tally-v4";
...
/** Jar3D: 3D-style glass jar with fill level (0..1) */
...
function Modal({ open, onClose, title, children, actions }) {
  ...
  const Card = ({ children }) => (
    <div className="bg-white rounded-2xl shadow-2xl border border-slate-200">{children}</div>
  );
  if (Motion) {
    return (
      <Backdrop>
        ...
        <div className="max-w-6xl mx-auto px-3 sm:px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1 className="text-xl sm:text-2xl font-bold">
              Mason Jar Bean Tally <span className="align-middle text-xs font-normal px-2 py-1 rounded-full bg-emerald-100 text-emerald-800 border border-emerald-200">v4</span>
            </h1>

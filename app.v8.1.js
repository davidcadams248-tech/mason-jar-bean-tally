// Mason Jar Bean Tally - v8.1 (self-contained)
// Based on v8; only change is wider player cards (approx 1.5x) by using fewer columns.
// - Jar fill = total beans / 50 (clamped)
// - Jar liquid color = weighted mix of bean colors inside
// - Click jar to add selected bean
// - Custom beans editor, summary chart, per-player jar glass tint
...
const STORAGE_KEY = "mason-jar-bean-tally-v8.1";
...
function Jar3D({ level = 0, label = "", tint = "#dbeafe", liquid = "#c28e6b", onClick, size = "sm" }) {
  ...
  return (
    <button onClick={onClick} title="Click to drop selected bean" className="focus:outline-none focus:ring-2 focus:ring-slate-300 rounded-xl" style={{ lineHeight: 0 }}>
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
        ...
        <div className="max-w-6xl mx-auto px-3 sm:px-4 py-3 flex items-center gap-2 sm:gap-3">
          ...
          <div className="flex-1">
            <h1 className="text-lg sm:text-2xl font-bold">
              Mason Jar Bean Tally <span className="align-middle text-[10px] sm:text-xs font-normal px-1.5 py-0.5 rounded-full bg-emerald-100 text-emerald-800 border border-emerald-200">v8.1</span>
            </h1>
      ...
      <main className="max-w-6xl mx-auto px-2 sm:px-4 py-3 sm:py-4">
        <div className="grid sm:grid-cols-1 lg:grid-cols-[minmax(230px,300px)_1fr] gap-3">
          ...
          <section className="flex-1">
            ...
            {/* Wider cards: 2 columns on XL to give each card ~1.5x the width versus v8 */}
      ...
      >
        <div className="space-y-2">
          {beans.map((b) => (
            <div key={b.id} className="flex items-center gap-2 border border-slate-200 rounded-2xl p-2">

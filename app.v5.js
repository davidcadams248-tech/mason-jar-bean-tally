// Mason Jar Bean Tally - v5 (combined with v4 + new features)
// Keeps v4 features (custom beans editor, summary chart, 3D jars, mobile layout)
// Adds:
// 1) Per-player jar color
// 2) Jar fill = % of full jar (50 beans = 100%)
// 3) Compact bean controls: small +/- with bean swatch + count centered
// 4) Smaller +/- buttons
...
const STORAGE_KEY = "mason-jar-bean-tally-v5"; // bump to avoid clobbering old data
...
function Jar3D({ level = 0, label = "", tint = "#dbeafe" }) {
...
function App() {
  ...
  function levelFor(p) {
    // v5: absolute scale — 50 beans is full jar
    return Math.max(0, Math.min(1, totalFor(p) / JAR_MAX_BEANS));
  }
        ...
        <div className="max-w-6xl mx-auto px-3 sm:px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1 className="text-xl sm:text-2xl font-bold">
              Mason Jar Bean Tally <span className="align-middle text-xs font-normal px-2 py-1 rounded-full bg-emerald-100 text-emerald-800 border border-emerald-200">v5</span>
            </h1>
            <p className="text-xs sm:text-sm text-slate-600">50 beans = full jar • Customize player jar color in each card.</p>
          </div>
      ...
      <main className="max-w-6xl mx-auto px-2 sm:px-4 py-4">
        <div className="grid sm:grid-cols-1 lg:grid-cols-[minmax(260px,360px)_1fr] gap-3">
                ...
                {players.map((p) => {
                  ...
                  return (
                    <li key={p.id} className="flex items-center gap-3">
                      <Jar3D level={level} tint={p.jarColor || "#dbeafe"} label={p.name + ' total jar'} />
          ...
          <section className="flex-1">
            ...
            {/* No-overlap, compact row controls */}
                  ...
                  <div key={p.id} className="group relative bg-white border rounded-2xl p-4 shadow-sm border-slate-200">
                    <div className="flex items-start gap-3">
                      <div className="flex flex-col items-center gap-1">
                        <Jar3D level={levelFor(p)} tint={p.jarColor || "#dbeafe"} label={p.name + ' card jar'} />
                        <label className="text-[10px] text-slate-500 -mt-1">Jar color</label>
                        <input
                          type="color"
                          value={p.jarColor || "#dbeafe"}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {/* Bean rows: small +/-, center swatch+count */}
                        <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
                          {state.beans.map((b) => (
                            <div key={b.id} className="flex items-center justify-between gap-2 rounded-xl border border-slate-200 px-2 py-1.5">
                              <button onClick={() => changeCount(p.id, b.id, -1)} className="px-2 py-1 rounded bg-white border text-xs hover:bg-slate-50">−</button>
                              <div className="flex items-center gap-2 min-w-0 flex-1 justify-center">
                                <BeanIcon hex={b.hex} />
                                <span className="text-sm truncate">{b.name}</span>
                                <span className="w-8 text-center font-mono text-sm tabular-nums">{p.counts[b.id] || 0}</span>
                              </div>
                              <button onClick={() => changeCount(p.id, b.id, 1)} className="px-2 py-1 rounded bg-white border text-xs hover:bg-slate-50">+</button>
                            </div>
                          ))}
                        ...
                        <div className="mt-3 flex items-center justify-between">
                          <div className="text-xs text-slate-500">Tip: 50 beans = full jar</div>
                          <div className="flex items-center gap-2">
                            <button onClick={() => { if (confirm('Reset this jar?')) resetPlayer(p.id); }} className="px-2 py-1 rounded-lg bg-white border border-slate-200 hover:bg-slate-100 text-sm">Reset</button>

// Mason Jar Bean Tally - v9
// Goal: Enlarge player title cards to avoid overlap. Slightly enlarge jars & bean icons.
// Keeps all v8 features: jar color mixes from bean population, total/50 fill, palette click-to-jar, etc.
...
const STORAGE_KEY = "mason-jar-bean-tally-v9";
...
/* ---------------------------------- SVGs ---------------------------------- */
function BeanIcon({ hex, className = "w-4.5 h-4.5" }) {
  // Tailwind doesn't have 4.5, but custom strings pass through; fallback to w-5 h-5 if needed.
  return (
...
function Jar3D({ level = 0, label = "", tint = "#dbeafe", liquid = "#c28e6b", onClick, size = "md" }) {
  const lvl = Math.max(0, Math.min(1, level));
  const fillTop = 120 - lvl * 84;
  const cls = size === "lg" ? "w-24 h-[140px]" : size === "md" ? "w-20 h-[115px]" : size === "sm" ? "w-16 h-[92px]" : "w-14 h-[78px]";
  const liquidDark = darken(liquid, 0.72);
  return (
    <button
      onClick={onClick}
  ...
  return (
    <button
      ...
      className={(active ? "bg-slate-900 text-white border-slate-900 " : "bg-white border-slate-200 ") +
                 "px-3 py-1.5 rounded-full border text-sm flex items-center gap-2 transition shadow-sm hover:shadow"}
    >
      {children}
    </button>
  );
...
function newPlayer(name, beans) {
  ...
  return { id: uid(), name, counts, jarColor: "#dbeafe" };
}
...
function App() {
  ...
  function liquidColorFor(p) {
    ...
    if (parts.length === 0) return "#cbd5e1";
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <header className="sticky top-0 z-10 backdrop-blur bg-slate-50/80 border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 py-3 flex items-center gap-3">
          <Jar3D level={0.2} tint="#dbeafe" size="sm" />
          <div className="flex-1">
            <h1 className="text-xl sm:text-3xl font-bold">
              Mason Jar Bean Tally <span className="align-middle text-xs sm:text-sm font-normal px-2 py-1 rounded-full bg-emerald-100 text-emerald-800 border border-emerald-200">v9</span>
            </h1>
            <p className="text-xs sm:text-sm text-slate-600">Pick a bean, then click jars to drop it • Jar color = mix of beans • 50 beans = full.</p>
          </div>
          ...
          <section>
            <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
              <div className="flex items-center justify-between mb-3">
                <h2 className="font-semibold text-base">Bean palette</h2>
                <button onClick={() => setBeansOpen(true)} className="text-xs px-2.5 py-1.5 rounded-lg bg-slate-900 text-white shadow hover:opacity-90">
                  Customize
                </button>
              </div>
              <div className="flex flex-wrap gap-2">
                {beans.map((b) => (
                  <Chip key={b.id} active={selectedBean === b.id} onClick={() => setSelectedBean(b.id)}>
                    <BeanIcon hex={b.hex} className="w-5 h-5" />
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm mt-4">
              <div className="flex items-center justify-between mb-2">
                <h2 className="font-semibold text-base">Leaderboard</h2>
                <button onClick={() => setChartOpen(true)} className="text-xs px-2.5 py-1.5 rounded-lg border border-slate-300 bg-white hover:bg-slate-50">
                  Summary chart
                </button>
              </div>
          ...
          <section className="flex-1">
            <div className="flex items-center gap-3 mb-4">
              <input
                ...
                className="flex-1 px-4 py-3 rounded-xl border border-slate-300 bg-white shadow-inner"
                onKeyDown={(e) => { if (e.key === 'Enter') addPlayer(newName); }}
              />
              <button onClick={() => addPlayer(newName)} className="px-4 py-3 rounded-xl bg-slate-900 text-white shadow hover:opacity-90">Add jar</button>
            </div>

            {players.length === 0 && (
              <div className="p-8 text-center text-slate-600 bg-white border border-dashed rounded-2xl">No jars yet. Add one above.</div>
            )}

            {/* Enlarged cards: 2 per row on desktop for more width */}
            <div className="grid sm:grid-cols-1 xl:grid-cols-2 gap-4">
              {players.map((p) => {
                ...
                const liquid = liquidColorFor(p);
                return (
                  <div key={p.id} className="group relative bg-white border rounded-2xl p-5 shadow-sm border-slate-200">
                    <div className="flex items-start gap-4">
                      <div className="flex flex-col items-center gap-2 w-24">
                        <Jar3D
                          ...
                          size="lg"
                        />
                        <label className="text-[11px] text-slate-500 -mt-1">Jar color</label>
                        <input
                          type="color"
                          value={tint}
                          onChange={(e) => setPlayerColor(p.id, e.target.value)}
                          className="w-10 h-7 p-0 border rounded"
                          title="Choose jar color"
                        />
                        {/* Stacked Reset/Remove */}
                        <div className="mt-1 flex flex-col gap-1 w-full">
                          <button onClick={() => { if (confirm('Reset this jar?')) resetPlayer(p.id); }} className="px-3 py-1.5 rounded bg-white border border-slate-200 hover:bg-slate-100 text-xs">Reset</button>
                          <button onClick={() => removePlayer(p.id)} className="px-3 py-1.5 rounded bg-white border border-rose-200 hover:bg-rose-50 text-rose-700 text-xs">Remove</button>
                        </div>
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <input
                          ...
                          className="w-full text-xl font-semibold bg-transparent outline-none border-b border-transparent focus:border-slate-300"
                        />
                        ...
                        <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-2">
                          {state.beans.map((b) => {
                            ...
                            return (
                              <div key={b.id} className="flex items-center justify-between gap-2 rounded-2xl border border-slate-200 px-2.5 py-1.5">
                                {/* Buttons larger tap area but borderless */}
                                <button onClick={() => changeCount(p.id, b.id, -1)} className="px-2.5 py-1 rounded text-sm hover:bg-slate-50">−</button>
                                {/* Center chip with count */}
                                <div className="relative flex-1 flex items-center justify-center">
                                  <div className="px-3 py-1 rounded-full" style={{ background: b.hex }}>
                                    <div className="flex items-center gap-2">
                                      <BeanIcon hex={b.hex} className="w-5 h-5" />
                                      <span className="text-sm truncate max-w-[9rem]" style={{ color: textColor }}>{b.name}</span>
                                      <span className="w-9 text-center font-mono text-sm tabular-nums" style={{ color: textColor }}>{count}</span>
                                    </div>
                                  </div>
                                </div>
                                <button onClick={() => changeCount(p.id, b.id, 1)} className="px-2.5 py-1 rounded text-sm hover:bg-slate-50">+</button>
        ...
        <div className="space-y-2">
          {beans.map((b) => (
            <div key={b.id} className="flex items-center gap-3 border border-slate-200 rounded-xl p-3">
              <div className="w-9 h-9 rounded-full border" style={{ background: b.hex }}></div>
              <input
                value={b.name}
                onChange={(e) => updateBean(b.id, { name: e.target.value })}
                className="flex-1 px-2 py-1.5 rounded-lg border border-slate-300"
              />
              <input
                type="color"
                value={b.hex}
                onChange={(e) => updateBean(b.id, { hex: e.target.value })}
                className="w-11 h-10 p-0 border rounded-lg"
                title="Pick color"
              />
              <button onClick={() => removeBean(b.id)} className="px-3 py-1.5 rounded-lg bg-white border border-rose-200 text-rose-700 hover:bg-rose-50">Remove</button>
            </div>
          ))}
